...
def get_instances(coords, offset, semantic_prediction_logits, grouping_cfg, verticality_feat, tree_class_in_dataset, non_trees_label_in_grouping, not_assigned_label_in_grouping, start_num_preds):
    ...
    print("MASK count: ", np.sum(mask_cluster))
    print("\tTREE MASK count: ", torch.sum(tree_mask).item())
    print("\tVERTICAL MASK count: ", np.sum(vertical_mask))
    print("\tOFFSET MASK count: ", np.sum(offset_mask))
    print("="*10)
...
def propagate_preds_hash_full(coords, instance_preds, coords_to_return, hash_mapping):
    ...
    missing_count = 0
    for i, hash_value in enumerate(hash_values):
        indices = hash_mapping.get(hash_value)
        if indices is not None:
            idx_array = np.array(indices, np.int64)
            target_preds[idx_array] = instance_preds[i]
            not_yet_propagated[idx_array] = False
        else:
            missing_count += 1

    if missing_count > 0:
        print(f"⚠️  Warning: {missing_count} hashes not found in mapping")
